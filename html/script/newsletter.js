$.ajax({
    ...
    success: function(response){
        const newletter_list_table = document.getElementById("newletter_list_table");
        newletter_list_table.innerHTML = '';
        response.forEach(newsletter => {
            newletter_list_table.innerHTML += `
                <tr>
                    <td class="">${newsletter.subject}</td>
                    <td>
                        <p class="email-body-text">${newsletter.body}</p>
                    </td>
                    <td>21/04/2005, 23:34 pm</td>
                    <td class="action-btn-list">
                        ...
                        <a href="email-opened.html?id=${newsletter.id}" class="btn btn-primary">
